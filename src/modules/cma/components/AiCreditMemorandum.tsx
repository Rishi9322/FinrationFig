...
export function AiCreditMemorandum() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const stream = await streamCmaCreditOpinion({
        parsedData,
        computedData
      });
  ...
  // Determine risk level based on the generated text if possible (simple heuristic)
  let riskBadge = null;
  const lowerOpinion = creditOpinion.toLowerCase();
  if (lowerOpinion.includes("risk rating: high")) {
    riskBadge = <span className="cma-badge badge-red">HIGH RISK</span>;
  } else if (lowerOpinion.includes("risk rating: moderate")) {
    riskBadge = <span className="cma-badge badge-amber">MODERATE RISK</span>;
  } else if (lowerOpinion.includes("risk rating: low")) {
      ...
      >
        {creditOpinion || (
          <div style={{ color: '#64748B', fontStyle: 'italic', display: 'flex', height: '100%', alignItems: 'center', justifyContent: 'center' }}>
            Click "Generate Opinion" to run the OpenRouter AI model and stream the credit memorandum.
          </div>
        )}
      </div>
    </div>
