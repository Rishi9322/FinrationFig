  ...
  return (
    <div className="cma-form-container">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
        <h2 style={{ fontSize: '1.25rem', fontWeight: 600, color: 'var(--cma-text-strong)' }}>Export Pack</h2>
      ...
      <div style={{ marginBottom: '1.5rem' }}>
        <h1 style={{ fontSize: '1.5rem', color: 'var(--cma-text-strong)', marginBottom: '0.25rem' }}>
          {parsedData.company || 'Credit Analysis'}
        </h1>
        <p style={{ color: 'var(--cma-text-muted)', fontSize: '0.85rem' }}>
      ...
      {summary && (summary.recommendation || summary.ccLimit || summary.tlLimit) && (
        <section style={{ marginBottom: '1.5rem' }}>
          <h3 style={{ color: 'var(--cma-text-muted)', fontSize: '0.8rem', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.5rem' }}>
            Executive Summary
          </h3>
          <div style={{ display: 'flex', gap: '1.5rem', flexWrap: 'wrap' }}>
            <div>
              <div style={{ fontSize: '0.75rem', color: 'var(--cma-text-muted)' }}>Recommendation</div>
              <div style={{ fontSize: '1.1rem', fontWeight: 700, color: 'var(--cma-text-strong)' }}>{summary.recommendation ?? 'Pending'}</div>
            </div>
            {summary.ccLimit && (
              <div>
                <div style={{ fontSize: '0.75rem', color: 'var(--cma-text-muted)' }}>CC Limit</div>
                <div style={{ fontSize: '1rem', fontWeight: 600, color: 'var(--cma-text-strong)' }}>₹ {summary.ccLimit} Lakhs</div>
              </div>
            )}
            {summary.tlLimit && (
              <div>
                <div style={{ fontSize: '0.75rem', color: 'var(--cma-text-muted)' }}>Term Loan Eligibility</div>
                <div style={{ fontSize: '1rem', fontWeight: 600, color: 'var(--cma-text-strong)' }}>₹ {summary.tlLimit} Lakhs</div>
              </div>
            )}
          </div>
          {recommendation && recommendation.rationale && (
            <p style={{ color: 'var(--cma-text)', fontSize: '0.85rem', marginTop: '0.75rem' }}>{recommendation.rationale}</p>
          )}
          {recommendation && recommendation.conditionsPrecedent.length > 0 && (
            <div style={{ marginTop: '0.5rem' }}>
              <div style={{ fontSize: '0.7rem', color: 'var(--cma-text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Conditions Precedent</div>
              <ul style={{ margin: '0.2rem 0 0', paddingLeft: '1.1rem', color: 'var(--cma-text)', fontSize: '0.85rem' }}>
          ...
          {recommendation && recommendation.monitoringPoints.length > 0 && (
            <div style={{ marginTop: '0.5rem' }}>
              <div style={{ fontSize: '0.7rem', color: 'var(--cma-text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Post-Disbursement Monitoring</div>
              <ul style={{ margin: '0.2rem 0 0', paddingLeft: '1.1rem', color: 'var(--cma-text)', fontSize: '0.85rem' }}>
      ...
      <section style={{ marginBottom: '1.5rem' }}>
        ...
        {creditOpinion ? (
          <div style={{ color: 'var(--cma-text)', lineHeight: 1.6, fontSize: '0.95rem' }}>
            <Markdown remarkPlugins={[remarkGfm]}>{creditOpinion}</Markdown>
          </div>
        ) : (
          <p style={{ color: 'var(--cma-text-faint)', fontStyle: 'italic' }}>
            No memo generated yet - go to "Generate Credit Note" first.
          </p>
        )}
      </section>

      <footer style={{ borderTop: '1px solid var(--cma-border)', paddingTop: '0.75rem', fontSize: '0.75rem', color: 'var(--cma-text-faint)' }}>
