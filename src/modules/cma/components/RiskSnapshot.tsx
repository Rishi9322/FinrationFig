...
export function RiskSnapshot() {
  ...
  return (
    ...
    <div
      style={{
        backgroundColor: 'var(--cma-panel-bg)', border: `1px solid ${severityColor(overall)}55`,
        borderRadius: '8px', padding: '1.25rem', marginBottom: '1rem',
        display: 'flex', flexWrap: 'wrap', gap: '1.5rem', alignItems: 'flex-start',
      }}
    >
      <div style={{ minWidth: '160px' }}>
        <div style={{ fontSize: '0.75rem', color: 'var(--cma-text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Risk Picture</div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginTop: '0.25rem' }}>
          <span style={{ width: 10, height: 10, borderRadius: '50%', backgroundColor: severityColor(overall), display: 'inline-block' }} />
          <span style={{ fontSize: '1.1rem', fontWeight: 600, color: 'var(--cma-text-strong)' }}>
      ...
      <div style={{ display: 'flex', gap: '1.5rem', flexWrap: 'wrap' }}>
        {[
          ...
          <div key={m.label}>
            <div style={{ fontSize: '0.75rem', color: 'var(--cma-text-muted)' }}>{m.label}</div>
      ...
      <div style={{ flex: '1 1 260px', minWidth: '260px' }}>
        <div style={{ fontSize: '0.75rem', color: 'var(--cma-text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.35rem' }}>
          Top Concerns
        </div>
        {concerns.length === 0 ? (
          <div style={{ color: 'var(--cma-text-faint)', fontSize: '0.85rem' }}>No threshold breaches on latest-year ratios.</div>
        ) : (
    ...
    {cashflowQuality && (
      <div style={{
        ...
        <div style={{ fontSize: '0.75rem', color: 'var(--cma-text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.6rem' }}>
