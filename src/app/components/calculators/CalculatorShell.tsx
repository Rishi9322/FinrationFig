    ...
    <div className="min-h-screen bg-background py-8" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-6">
          <Link
            to="/calculators"
            className="inline-flex items-center text-sm text-muted-foreground hover:text-link transition-colors mb-4 group"
          >
            <ChevronLeft className="h-4 w-4 mr-1 transition-transform group-hover:-translate-x-0.5" />
            All Calculators
          </Link>
          <div className="flex items-center gap-2 mb-3">
            <span className="text-xs font-['Geist_Mono'] text-muted-foreground">calculators</span>
            <span className="text-muted-foreground/40">/</span>
            <span className="text-xs font-['Geist_Mono'] text-link">{title.toLowerCase().replace(/\s+/g, "-")}</span>
          </div>
          <h1
        ...
        <div className="bg-card border border-foreground/8 rounded-xl mb-6 overflow-hidden">
          <div className={`grid grid-cols-1 ${videoUrl ? "md:grid-cols-2" : ""}`}>
            <div className={`p-6 md:p-8 flex flex-col justify-center ${videoUrl ? "md:border-r border-foreground/8" : ""}`}>
              <p className="text-xs font-['Geist_Mono'] text-link uppercase tracking-widest mb-3">
            ...
            {videoUrl && (
              <Dialog>
                <DialogTrigger asChild>
                  <button
                    className="relative min-h-[200px] md:min-h-0 w-full bg-secondary flex flex-col items-center justify-center gap-3 cursor-pointer group"
                  >
                    {/* Subtle grid */}
                    <div
                    ...
                    <div className="relative z-10 flex flex-col items-center gap-3">
                      <div className="relative">
                        <div className="absolute inset-0 rounded-full bg-primary/20 scale-125 group-hover:scale-150 transition-transform duration-500" />
                        <div className="w-14 h-14 rounded-full border-2 border-primary/60 group-hover:border-primary bg-primary/10 group-hover:bg-primary/20 flex items-center justify-center transition-all duration-200 relative z-10">
                          <PlayCircle className="w-7 h-7 text-link fill-primary/20" />
                        </div>
                      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* Input panel */}
          <div className="bg-card rounded-xl border border-foreground/8 p-6">
            <div className="flex items-center gap-2 mb-5 pb-4 border-b border-foreground/8">
              <div className="w-1.5 h-5 rounded-full bg-primary" />
              <h2 className="text-sm font-medium text-foreground">Input Values</h2>
            </div>
            {children}
          </div>
